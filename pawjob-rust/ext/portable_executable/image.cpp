...
std::uint8_t* portable_executable::image_t::find_export(const std::uint16_t ordinal) const
{
	...
	const std::uint16_t relative_ordinal = ordinal - static_cast<std::uint16_t>(export_directory->base);

	if (relative_ordinal >= export_directory->number_of_functions)
	{
		return nullptr;
	}

	const auto functions = reinterpret_cast<const std::uint32_t*>(module + export_directory->address_of_functions);

	const std::uint32_t function_rva = functions[relative_ordinal];
	...
	return const_cast<std::uint8_t*>(module + function_rva);
}
